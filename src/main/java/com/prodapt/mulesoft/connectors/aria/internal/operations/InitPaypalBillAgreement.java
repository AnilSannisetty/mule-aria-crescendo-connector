...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.InitPaypalBillAgreementParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class InitPaypalBillAgreement {
	private static final Logger logger = LoggerFactory.getLogger(InitPaypalBillAgreement.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Initialize Paypal Bill Agreement")
	@MediaType(value = "application/json")
	@Summary("This operation creates a session with PayPal so that an account user can set up a billing agreement with an Aria client. The operation returns a session token from PayPal, plus a client configured return_url (aka PayPal Agreement Confirmation URL) to instruct the USS/Reg software where it should direct the end user for the next step. PayPal will consider the token valid for 15 minutes.")
	public void initPaypalBillAgreement(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") InitPaypalBillAgreementParam ar,
		...
		String apiName = "init_paypal_bill_agreement_m";
	...
	private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, InitPaypalBillAgreementParam ar) {
