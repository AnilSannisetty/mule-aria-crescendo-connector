...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetAllClientReceiptIdsParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetAllClientReceiptIds {
	private static final Logger logger = LoggerFactory.getLogger(GetAllClientReceiptIds.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get All Client Receipt IDs")
	@MediaType(value = "application/json")
	@Summary("This operation returns a list of all client receipt IDs entered as input parameters for any API call to Aria. If the client_receipt hash doesn't display an account number, it means that the receipt ID didn't have an account number in scope or was linked to a call that included multiple account IDs.")
	public void getAllClientReceiptIds(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetAllClientReceiptIdsParam ar,
		...
		String apiName = "get_all_client_receipt_ids_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetAllClientReceiptIdsParam ar) {
