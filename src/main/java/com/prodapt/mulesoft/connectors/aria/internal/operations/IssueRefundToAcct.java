...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.IssueRefundToAcctParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class IssueRefundToAcct {
	private static final Logger logger = LoggerFactory.getLogger(IssueRefundToAcct.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Issue Refund To Account")
	@MediaType(value = "application/json")
	@Summary("This operation issues refunds of payment amounts and any invoice line items to be reversed for a specified account."
			+ " The refund of a payment will increase the account/master plan instance balance to its"
			+ " pre-payment state (or near, if a partial refund). A refund related reversal would almost retain the balance \"as is\", "
			+ "as you are reversing the invoice line item charges, along with the refund. Reversing an invoice charge line item"
			+ " proportionately reverses the corresponding tax (tax to reverse = original tax - tax for reversed charge). "
			+ "In the case of tax-inclusive charges, the specified line item reversal amount will be considered as a tax-inclusive reversal amount."
			+ " In the case of external tax engines, there will be a tax posting that will occur to update the new taxes to the"
			+ " external tax engine against the customer's account. As an aside, tax-posting to external tax engines occurs for all generated invoices, "
			+ "voided invoices, write-offs, refund-related reversals, proration reversal/credit (and credit-memo/rebills).")
	public void issueRefundToAcct(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") IssueRefundToAcctParam ar,
		...
		String apiName = "issue_refund_to_acct_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, IssueRefundToAcctParam ar) {
