...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetAcctStatementHistoryParam;

import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetAcctStatementHistory {
	private static final Logger logger = LoggerFactory.getLogger(GetAcctStatementHistory.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Account Statement History")
	@MediaType(value = "application/json")
	@Summary("This operation returns the list of all statements in a given billing period on an account for all the master plan instances or on a specific master plan instance on the account.")
	public void getAcctStatementHistory(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetAcctStatementHistoryParam ar,
		...
		String apiName = "get_acct_statement_history_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetAcctStatementHistoryParam ar) {
