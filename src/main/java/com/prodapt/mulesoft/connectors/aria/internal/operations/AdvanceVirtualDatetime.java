...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.AdvanceVirtualDatetimeParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class AdvanceVirtualDatetime {
	private static final Logger logger = LoggerFactory.getLogger(AdvanceVirtualDatetime.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Advance Virtual Datetime")
	@MediaType(value = "application/json")
	@Summary("This operation takes a number of hours by which to further advance virtual time for the given client, then returns the same output elements as \"get_virtual_datetime\", as well as the new/current offset hours value AFTER application of the input offset hours value. Use of this method in production systems will return an \"operation not allowed\" error as virtual time is only supported in non-production systems.")
	public void advanceVirtualDatetime(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") AdvanceVirtualDatetimeParam ar,
		...
		String apiName = "advance_virtual_datetime_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, AdvanceVirtualDatetimeParam ar) {
