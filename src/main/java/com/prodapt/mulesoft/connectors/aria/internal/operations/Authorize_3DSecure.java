...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.Authorize_3DSecureParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class Authorize_3DSecure {
	private static final Logger logger = LoggerFactory.getLogger(Authorize_3DSecure.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Authorize 3D Secure")
	@MediaType(value = "application/json")
	@Summary("This operation authorizes a credit or debit card if you specified that 3D Secure (3DS) is required "
			+ "for a particular transaction and the card is enrolled in 3DS; for example, an order over a particular amount that you specify."
			+ " The 3DS authentication feature provides additional fraud prevention when transactions are processed."
			+ " This feature requires account holders to provide additional information such as a password when they make an online purchase using a credit or debit card.")
			
	public void authorize_3DSecure(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") Authorize_3DSecureParam ar,
		...
		String apiName = "authorize_3dsecure_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, Authorize_3DSecureParam ar) {
