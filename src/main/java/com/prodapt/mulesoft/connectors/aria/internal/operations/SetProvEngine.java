...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.SetAcctTaxExemptStatusParam;
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.SetProvEngineParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class SetProvEngine {
	private static final Logger logger = LoggerFactory.getLogger(SetProvEngine.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Set Provisioning Engine")
	@MediaType(value = "application/json")
	@Summary("This operation specifies whether a client should receive event notification messages. If you disable event notification, this simply stops the transmission of the messages but does not prevent the event notification records from being created. If event notification is re-enabled, then the client will receive queued notification messages in the order in which they were created.")
	public void setprovengine(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") SetProvEngineParam ar,
		...
		String apiName = "set_prov_engine_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, SetProvEngineParam ar) {
