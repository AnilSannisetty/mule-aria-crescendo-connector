...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.CopyAcctPaymentMethodParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class CopyAcctPaymentMethod {
	private static final Logger logger = LoggerFactory.getLogger(CopyAcctPaymentMethod.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Copy Account Payment Method")
	@MediaType(value = "application/json")
	@Summary("This operation copies one or more payment methods (and their associated billing contact information) from one customer account in Aria to a different customer account. Clients must validate that the account owner is the same for both the source and target accounts prior to performing this operation. Note that payment methods copied to the target account are not by default associated with a billing group. If no payment methods are specified, all non-disabled payment methods on the source account are copied to the target account.")
	public void copyAcctPaymentMethod(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") CopyAcctPaymentMethodParam ar,
		...
		String apiName = "copy_acct_payment_method_m";
	...
	private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, CopyAcctPaymentMethodParam ar) {
