...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetCatalogHierarchyParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetCatalogHierarchy{
	private static final Logger logger = LoggerFactory.getLogger(GetCatalogHierarchy.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Catalog Hierarchy")
	@MediaType(value = "application/json")
	@Summary("This operation can be used to fetch the hierarchy of plans for a given client. It provides the ability to filter the results to return the catalog hierarchy by promo code, parent plan number, client parent plan ID. Each filter can be used independently or in combination to provide control on reading the catalog. The intent of this operation is to give a developer more control on reading the plan hierarchy.")
	public void getCatalogHierarchy(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetCatalogHierarchyParam ar,
		...
		String apiName = "get_catalog_hierarchy_m";
