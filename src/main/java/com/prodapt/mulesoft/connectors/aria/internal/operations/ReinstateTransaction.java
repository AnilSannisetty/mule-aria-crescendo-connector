...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.ReinstateTransactionParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class ReinstateTransaction {
	private static final Logger logger = LoggerFactory.getLogger(ReinstateTransaction.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Reinstate Transaction")
	@MediaType(value = "application/json")
	@Summary("This operation creates a new transaction of the type \"Reinstated Charge Transaction\" or \"Reinstated Payment Transaction\". The new transaction corresponds to a previously voided transaction associated with a specified account number and transaction ID. Note that neither the original transaction nor the voided transaction are modified by this operation.")
	public void reinstateTransaction(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") ReinstateTransactionParam ar,
		...
		String apiName = "reinstate_transaction_m";
	...
	private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, ReinstateTransactionParam ar) {
