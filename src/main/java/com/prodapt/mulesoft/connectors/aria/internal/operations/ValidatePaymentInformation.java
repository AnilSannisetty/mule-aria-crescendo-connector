...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.ValidatePaymentInformationParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class ValidatePaymentInformation {
	private static final Logger logger = LoggerFactory.getLogger(ValidatePaymentInformation.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Validate Payment Information")
	@MediaType(value = "application/json")
	@Summary("This operation determines the validity of a card by performing an authorization transaction. "
			+ "The default authorization amount is 0 or 1 unit of currency (for example, 0 or $1 USD)"
			+ " depending on payment processor and card type. Please note: In order to use this operation, "
			+ "your payment processor must support card authorizations.")
	public void validatePaymentInformation(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") ValidatePaymentInformationParam ar,
		...
		String apiName = "validate_payment_information_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, ValidatePaymentInformationParam ar) {
