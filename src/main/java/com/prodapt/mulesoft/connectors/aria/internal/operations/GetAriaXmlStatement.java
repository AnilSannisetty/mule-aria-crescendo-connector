...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetAriaXmlStatementParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetAriaXmlStatement {
	private static final Logger logger = LoggerFactory.getLogger(GetAriaXmlStatement.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Aria XML Statement")
	@MediaType(value = "application/json")
	@Summary("Use this operation to return the contents of a specific XML statement for an account, transaction history of parent, and child accounts. The XML statement is generated as an auxiliary statement along with the regular statement when the \"XML Distillation Option\" client setting is turned on (choosing with or without usage details). The XML statement generated provisioning event returns the xml_statement_no in its event payload, and is used as an input variable in this operation (API call).")
	public void getAriaXmlStatement(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetAriaXmlStatementParam ar,
		...
		String apiName = "get_aria_xml_statement_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetAriaXmlStatementParam ar) {
