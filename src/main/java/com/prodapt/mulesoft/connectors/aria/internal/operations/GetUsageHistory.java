...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetUsageHistoryParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetUsageHistory {
	private static final Logger logger = LoggerFactory.getLogger(GetUsageHistory.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Usage History")
	@MediaType(value = "application/json")
	@Summary("This operation returns usage records for an account, and can be filtered by date range, usage type, invoice, exclude status, and qualifiers. Using limit and offset inputs, results can be paginated.")
	public void getUsageHistory(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetUsageHistoryParam ar,
		...
		String apiName = "get_usage_history_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetUsageHistoryParam ar) {
