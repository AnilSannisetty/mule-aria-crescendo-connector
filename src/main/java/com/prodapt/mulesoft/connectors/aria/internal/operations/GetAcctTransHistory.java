...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetAcctTransHistoryParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetAcctTransHistory {
	private static final Logger logger = LoggerFactory.getLogger(GetAcctTransHistory.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Account Transaction History")
	@MediaType(value = "application/json")
	@Summary("This operation returns a list of financial transactions associated with an account. You can filter the transactions using criteria such as master plan instance, transaction type, date range, or statement number.")
	public void getAcctTransHistory(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetAcctTransHistoryParam ar,
		...
		String apiName = "get_acct_trans_history_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetAcctTransHistoryParam ar) {
