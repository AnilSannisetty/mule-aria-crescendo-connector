...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetClientPlansAllParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetClientPlansAll {
	private static final Logger logger = LoggerFactory.getLogger(GetClientPlansAll.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Client Plans All")
	@MediaType(value = "application/json")
	@Summary("This operation returns a detailed list of all plans associated with a client. If you do not filter the call by acct_no, the information returned includes the services and rate schedules associated with the plans. If you filter the call by acct_no, or enter a plan_no or a client_plan_id that is a supplemental plan number, the information returned does not include any master plan service or rate schedule details.")
	public void getClientPlansAll(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetClientPlansAllParam ar,
		...
		String apiName = "get_client_plans_all_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetClientPlansAllParam ar) {
