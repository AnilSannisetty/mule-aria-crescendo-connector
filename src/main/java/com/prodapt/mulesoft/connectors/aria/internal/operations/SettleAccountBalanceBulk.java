...
public class SettleAccountBalanceBulk {
	...
	@DisplayName("CRUD:Settle Account Balance Bulk")
	@MediaType(value = "application/json")
	@Summary("This API allows for the batch submission of multiple accounts to a payment processor for collection attempts on each account. Each submitted account must be assigned to a collection group before running this API, or the collection does not take place. The supplied accounts are added to a flat file that is submitted to the payment processor. Once they make the required collection attempts, a response file is sent back to Aria and the associated accounts are updated.")
