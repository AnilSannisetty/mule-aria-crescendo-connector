...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.SettleAccountBalanceBulkParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class SettleAccountBalanceBulk {
	private static final Logger logger = LoggerFactory.getLogger(SettleAccountBalanceBulk.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Settle Account Balance Bulk")
	@MediaType(value = "application/json")
	@Summary("This operation allows for the batch submission of multiple accounts to a payment processor for collection attempts on each account. Each submitted account must be assigned to a collection group before running this operation, or the collection does not take place. The supplied accounts are added to a flat file that is submitted to the payment processor. Once they make the required collection attempts, a response file is sent back to Aria and the associated accounts are updated.")
	public void settleAcctBalanceBulk(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") SettleAccountBalanceBulkParam ar,
		...
		String apiName = "settle_account_balance_bulk_m";
	...
	private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, SettleAccountBalanceBulkParam ar) {
