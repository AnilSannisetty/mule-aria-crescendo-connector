...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.SetUsageThresholdParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class SetUsageThreshold {
	private static final Logger logger = LoggerFactory.getLogger(SetUsageThreshold.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Set Usage Threshold")
	@MediaType(value = "application/json")
	@Summary("This operation sets the Billing-Period-To-Date or Month-To-Date unbilled usage threshold amount"
			+ " on the master plan instance for client level event notifications or "
			+ "master plan instance level event notifications. In addition, the operation also allows setting threshold levels.")
	public void setUsageThresholdParam(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") SetUsageThresholdParam ar,
		...
		String apiName = "set_usage_threshold_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, SetUsageThresholdParam ar) {
