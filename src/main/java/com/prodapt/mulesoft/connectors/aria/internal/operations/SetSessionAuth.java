...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.SetSessionAuthParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class SetSessionAuth {
	private static final Logger logger = LoggerFactory.getLogger(SetSessionAuth.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Set Session Authentication")
	@MediaType(value = "application/json")
	@Summary("This operation authenticates a customer who logs into a User Self Service application with a user ID and password. Starts an Aria session for the customer that is logged in. Within the Aria session, you can perform functions such as securely processing payment transactions or securely retrieving account information.")
	public void setsessionauth(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") SetSessionAuthParam ar,
		...
		String apiName = "set_session_auth_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, SetSessionAuthParam ar) {
