...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.GetAcctPlanHistoryParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class GetAcctPlanHistory {
	private static final Logger logger = LoggerFactory.getLogger(GetAcctPlanHistory.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Get Account Plan History")
	@MediaType(value = "application/json")
	@Summary("The operation returns a list of plan changes (including assignments, replacements, status changes, etc.) for the specified account.")
	public void getAcctPlanHistory(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") GetAcctPlanHistoryParam ar,
		...
		String apiName = "get_acct_plan_history_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, GetAcctPlanHistoryParam ar) {
