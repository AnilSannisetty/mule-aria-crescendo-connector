...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.CancelAcctPlanParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class CancelAcctPlan {
	...
	@Summary("This operation cancels a master or supplemental plan assigned to a specified account. "
			...
			+ "These child plans however, remain on the account in an unusable state in the event the parent plan is re-activated. "
			+ "When all master plans on an account have a plan status of Cancelled, Terminated or Suspended, "
			+ "the account will be automatically deactivated based on the Days Before Deactivation configuration parameter.")
	public void cancelAcctPlan(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") CancelAcctPlanParam ar,
		...
		String apiName = "cancel_acct_plan_m";
...
private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, CancelAcctPlanParam ar) {
