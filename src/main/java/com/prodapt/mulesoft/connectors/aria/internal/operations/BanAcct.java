...
import com.prodapt.mulesoft.connectors.aria.internal.operations.parametergroup.BanAcctParam;
import com.prodapt.mulesoft.connectors.aria.internal.utility.RestRequestBuilder;

public class BanAcct {
	private static final Logger logger = LoggerFactory.getLogger(BanAcct.class);
	public static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT = RestRequestBuilder.QueryParamFormat.MULTIMAP;

	@DisplayName("Ban Account")
	@MediaType(value = "application/json")
	@Summary("Sets an account's status to temporary ban and, optionally, specifies when the ban will be removed and the account returned to its previous status: either on a specific date (date_to_restore) or after a specified number of days (days_to_restore).")
	public void banAcct(@Config RestConfiguration rc, @Connection AriaConnection ac,
			@ParameterGroup(name = "Input Arguments") BanAcctParam ar,
	...
	private JSONObject attachStandardParamsToPayload(TypedValue<InputStream> payload, String apiName, AriaConnection ac, BanAcctParam ar) {
